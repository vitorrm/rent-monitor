...
async function parseHouses (page) {
	...
	for (let i = 0; i < cards.length; i++) {
		const img = await cards[i].$eval('.fotorama__active > img', i => i.getAttribute('src'))
		const href = await cards[i].$eval('a', a => a.getAttribute('href'))
		let value = await cards[i].$eval('.valor h5', i => i.innerText)
		const location = await cards[i].$eval('.localizacao > span', i => i.innerText)
		try {
